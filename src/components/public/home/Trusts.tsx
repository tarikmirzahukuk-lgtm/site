    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="text-center">
          ...
          <h2
            className="display-monument m-0 mx-auto max-w-[760px] stagger-item"
            style={{ fontSize: "clamp(30px, 4vw, 44px)", ["--i" as string]: 0 }}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3.5 md:gap-5 mt-9 md:mt-14">
          {data.items.map((t, i) => (
            <div
              key={i}
              className="tablet-card p-6 md:p-7 relative stagger-item"
              style={{ ["--i" as string]: i + 1 }}
            >
              <span
                className="roman-watermark engrave-in"
                style={{ ["--i" as string]: i + 1 }}
                aria-hidden="true"
              >
                {toRoman(i + 1)}
